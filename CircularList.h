...
void CircularList<T>::remove(int index) 
{
   ...
   if (index >= 1 && index <= sze) 
   {
      ...
      if (sze == 1) //special case
      {
		...
		DoubleNode<T>* node = loc;   //makes the loc the head
		node = NULL;                 //makes node = to nothing
      ...
      else
      {
		...
		//to do update loc and loc_pos!!!!!!!!!!
		DoubleNode<T>* node = loc;
		
		DoubleNode<T>* curr = find(index);         //finds what is wanted to remove
		DoubleNode<T>* prev = curr->getPrev(curr); //gets the address before current
		DoubleNode<T>* next = curr->getNext(curr); //gets the address after current
		
		prev->setNext(next); //links previous to next
		next->setPrev(prev); //Links next to previous
		
		delete curr; //deletes the desired element
		
		loc_pos = index;  //makes the local position equal to the index
		loc = next;       //makes loc equal to next
